...
TemplateNode::TemplateNode() : Node("template_node"), count_(0) {
  ...
  // Create publishers
  string_publisher_ = this->create_publisher<std_msgs::msg::String>(topic_prefix + "/status", 10);

  counter_publisher_ = this->create_publisher<std_msgs::msg::Int32>(topic_prefix + "/counter", 10);

  temperature_publisher_ =
      this->create_publisher<sensor_msgs::msg::Temperature>(topic_prefix + "/temperature", 10);
  ...
  timer_ = this->create_wall_timer(timer_period, std::bind(&TemplateNode::timer_callback, this));

  RCLCPP_INFO(this->get_logger(), "Template node initialized with rate: %.2f Hz", publish_rate);
...
void TemplateNode::timer_callback() {
  ...
  temp_msg.temperature = 20.0 + 5.0 * std::sin(count_ * 0.1);  // Simulated temperature
  temp_msg.variance = 0.1;
  temperature_publisher_->publish(temp_msg);

  if (count_ % 10 == 0) {
    RCLCPP_INFO(this->get_logger(), "Published count: %zu, temp: %.2f°C", count_,
