...
void PerformanceMonitor::record_callback_end() {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
        end_time - callback_start_time_).count() / 1000.0; // Convert to milliseconds
...
double PerformanceMonitor::get_cpu_usage() {
    ...
    if (time_diff > 500 && last_total_time > 0) { // Only calculate if >500ms passed
...
double PerformanceMonitor::calculate_publish_rate() {
    ...
    auto time_diff = std::chrono::duration_cast<std::chrono::seconds>(
        current_time - last_metrics_time_).count();
    
    if (time_diff <= 0) return 0.0;
    
    size_t current_count = total_messages_published_.load();
    double rate = static_cast<double>(current_count - last_published_count_) / time_diff;
...
void PerformanceMonitor::publish_metrics() {
    ...
    add_kv("cpu_percent", std::to_string(static_cast<double>(metrics_json["system"]["cpu_percent"])));
    add_kv("memory_mb", std::to_string(static_cast<size_t>(metrics_json["system"]["memory_mb"])));
    add_kv("uptime_seconds", std::to_string(static_cast<double>(metrics_json["system"]["uptime_seconds"])));
    add_kv("messages_published_total", std::to_string(static_cast<size_t>(metrics_json["ros"]["messages_published_total"])));
    add_kv("publish_rate_hz", std::to_string(static_cast<double>(metrics_json["ros"]["publish_rate_hz"])));
    add_kv("avg_callback_ms", std::to_string(static_cast<double>(metrics_json["ros"]["avg_callback_ms"])));
